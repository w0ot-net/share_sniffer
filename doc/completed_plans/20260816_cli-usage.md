...
Implemented 2026-08-16. Commit: `18a702d`.

- Replaced obsolete scanner examples with positional targets and valid concurrency and
  share-filter options, and documented final status, analyzer rendering, download
  naming, credential, and test contracts in the root README.
- Recorded executable modes for all three shebang-based entry points.
- Added `tests/__init__.py` as a bounded correction because the planned
  `python3 -m unittest discover -v` command otherwise discovered zero tests; no test
  framework or infrastructure was added.
- Follow-up commit `d73faa5` clarifies that scanner total failure returns `1` while
  argparse syntax errors retain their own nonzero usage status.
- Validation passed: direct `--help` for all three scripts, parser checks for every
  README example, `python3 -m unittest discover -v` (13 tests),
  `python3 -m compileall -q .`, and `git diff --check`.
- No unresolved items; CI, packaging, broad cleanup, and other explicit non-goals remain
  excluded.
